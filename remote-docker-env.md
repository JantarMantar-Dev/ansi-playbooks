...
The intended steady state is `node.role==worker` for stateless apps, with any unhealthy worker drained at the node level. Dokploy auto-deploys replace direct service placement overrides, but node `Drain` persists in Swarm. The four SSD workers and `racknerd-66b5b59` were repaired by worker-only rejoin with Tailscale advertise addresses, capacity cleanup where needed, and overlay VIP canaries; all are now `Active`. Follow [docs/swarm-post-recovery-validation.md](docs/swarm-post-recovery-validation.md) after every node restart or upgrade before returning a node to `Active`.
